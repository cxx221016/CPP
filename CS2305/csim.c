...
static inline uintptr_t getset(uintptr_t addr,int s,int b)
{
    return (addr>>b)&((1<<s)-1);
    //return (addr>>b)%((1<<s));
}

static inline uintptr_t gettag(uintptr_t addr,int s,int b)
{
    return addr>>(s+b);
}
...
{
    uintptr_t setindex = getset(op.addr,c->s,c->b);
    uintptr_t tag = gettag(op.addr,c->s,c->b);
...
int main(int argc, char **argv)
{
    ...
    while((curop=opinit(fp)).op!=0)
    {
        ...
        {
            //printf("L %lx,%d",curop.addr,curop.size);
            //printf("getop");
            LS(&c,curop,v,true);
        }
        else if(curop.op=='M')
        {
            //printf("M %lx,%d",curop.addr,curop.size);
            M(&c,curop,v);
        }
    }
    fclose(fp);
