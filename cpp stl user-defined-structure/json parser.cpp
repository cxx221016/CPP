...
int main()
{
    std::ifstream fin("D:/MY_CODE/CPP/user define datastructure/test.json");
    std::ofstream fout("D:/MY_CODE/CPP/user define datastructure/testout.json");
    std::stringstream ss;
    ss<<fin.rdbuf();
    std::string s{ss.str()};
    try 
    {
        ...
        x["version"] = { 114514LL };
        std::cout<<x<<std::endl;
        std::string res=JsonGenerator::generate(x);
        ss.str(res);
