...
string& Clear_Space(string &str)//去除前导空格
{
...
pair<string,string> regex_split(const string& cur)
{
    string pre_name="",name="",exp="",pre_exp="";
    regex pattern(".*?\\[(.*?)\\].*?");
	smatch results;
    if (regex_match(cur, results, pattern))
    {
        pre_name=results[1];
    }
    else return {"",""};
    ...
    name = Clear_Space(pre_name); //去除空格；
    regex exp_pattern(".*?\\](.*?);.*?");
    smatch exp_results;
    if(regex_match(cur,exp_results,exp_pattern))
    {
        pre_exp=exp_results[1];
    }
    else 
    {
        return {name,""};
    }
    exp= Clear_Space(pre_exp);  //去除空格；
    return {name,exp};
}

pair<string,string> calculate(const string& cur)
{
    ts=Token_stream();
    auto&& [name,exp]=regex_split(cur);
    if(name=="") return {"",""};
    if(exp=="") 
    {
        mp[name].clear();
...
//字符串预处理，检测q，离散
vector<string> Expression_preprocessing(string& exp_str)
{
    vector<string> init;
    ...
    if(!str.empty()) init.push_back(str);
    return init;
}
...
try 
{
    ...
    getline(cin,exp_str);
    getline(cin,query);
    init=Expression_preprocessing(exp_str);
    unordered_multiset<string> seen;
    string last;
    for(auto& exp:init)
    {
        auto pr=calculate(exp);
        os.str("");
        if(last=="")
        {
            if(pr.first.size()>0)
        ...
        else if(!last.empty())
        {
            if(pr.first.size()>0)
            {
                if(seen.size()>1)
                {
                    ...
                    {
                        //mp[last].erase(EP);
                        auto it=mp[last].find(EP);
                        if(it!=mp[last].end()) mp[last].erase(it);
                    }
                }
                seen.clear();
            }
