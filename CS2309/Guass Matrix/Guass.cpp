...
{
    int flag=x>=0;
    if(flag) return floor(x * 10000.000f+0.1) / 10000.000f;
...
bool isvalid(string& str,vector<pair<string,double>>& init,double& b)
{
    ...
    while(i<n)
    {
        ...
        else if(islower(ch))
        {
            ...
            if(cnt<0) return false;
            if(seen.count(xyz)) return false;
            seen.insert(xyz);
            init.push_back({xyz,isz*coef});
            isz=1;
            i=j;
...
int main()
{
    ...
    try
    {
        if(index==0&&n==0) throw MyClass3();
        if(index>n) throw MyClass2();
        for(int i=0;i<n;++i)
        {
            auto a=A[i];
            for(auto& entry:a)
        ...
        for(auto& see:seen)
        {
            cout<<endl<<see<<":"<<fixed<<setprecision(4)<<keep(result[i]);
            ++i;
        }
        suf_display();
    }
