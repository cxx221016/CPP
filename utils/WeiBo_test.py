...
def get_text(url):
    ...
    text = etree.HTML('/html/body/div[1]/div[1]/div/div[2]/div/article/div/div/div[1]/text()')
    print(text)
            ...
            if(len(cards)>0):
                for j in range(len(cards)):
                    ...
                    if(card_type==9):
                        ...
                        if(idx!=-1):
                            tmp=text[:idx]
                            idx=tmp.rfind('/status/')
                            tmp=tmp[idx:]
                            idx=tmp.find('"')
                            tmp=tmp[:idx]
                            href='https://m.weibo.cn'+tmp
                            print(href)
